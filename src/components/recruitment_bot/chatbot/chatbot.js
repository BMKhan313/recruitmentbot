...
const ChatBot = props => {
  ...
  useEffect(() => {
    ...
    if ((props.location.state === null) & (props.location.search === null)) {
      ...
      if (props.location.search !== "") {
        ...
        setToken(info[1])
        setUserId(info[2])
        CURRENT_API_URL = info[3] == "AlMEZ020" ? MEEZAN_API_URL : REC_URL

        red_url = info[4]
        setUserChatId(info[5])
        id_of_chat = info[5]
        // sessionStorage.setItem("user_chat_id",info[5])
        // sessionStorage.setItem("red_url",info[4])
        ...
        CURRENT_API_URL =
          props.location.state.company_code == "AlMEZ020"
            ? MEEZAN_API_URL
            : REC_URL
        setUserChatId(sessionStorage.getItem("user_chat_id"))
        id_of_chat = sessionStorage.getItem("user_chat_id")
    ...
    async function getCareerName() {
      //ANCHOR - Refactored- check-job-applied
      const career_name = await fetch(
        `${CURRENT_API_URL}/recruitment/check-job-applied?user_chat_id=${parseInt(
          id_of_chat
        )}`,
        {
          method: "GET",
    ...
    //ANCHOR - Refactored - previous-chat
    fetch(`${CURRENT_API_URL}/recruitment/previous?chat_key=${id_of_chat}`, {
      ...
      .then(res => {
        ...
        if (data?.previous_chat) {
          setState({ show1stPrompt: false })
        } else {
          setState({ show1stPrompt: true })
        }
                        ...
                        <div className={classes.position_right}>
                          ...
                          <div className={classes.radio_button_side}>
                            <div
                              ...
                              id="test_main_div"
                            >
                              {/* {clog({checkkk_First:  user_info.country})} */}
                              {state.gif === true
                                ? null
                                ...
                                ? salary_expectations[user_info.country] !==
                                  undefined
                                  ? salary_expectations[
                                      user_info.country
                                    ].option_name.map(
                    ...
                    {showRange === true ? (
                      <div className={classes.position_right}>
                        ...
                        <div className={classes.radio_button1}>
                          <div>
                            <div className={classes.range_display_div}>
                              <span className={classes.range_text1}>5</span>
                              <span className={classes.range_text2}>
                                {state.user_input}
                              </span>
                              <span className={classes.range_text3}>100</span>
                            </div>
                            <input
                              type="range"
                              name="age_range"
                            ...
                            <div className={classes.radio_button1}>
                              <div>
                                <div className={classes.range_display_div}>
                                  ...
                                  <span className={classes.range_text3}>
                                    100
                                  </span>
                                </div>
                                <input
  ...
  return (
    <div className={classes.container_div}>
      {state.show1stPrompt && (
        <div className={classes.first_prompt}>
          <ImCross
            className={classes.cross_icon}
            onClick={() => setState({ show1stPrompt: false })}
          />
          <h5 className={classes.welcome_header}>Welcome to RecruitmentBot!</h5>
          <p className={classes.description}>
            You will need to answer some questions so we can assess how suitable
            you are for certain job roles. You will not be allowed to edit your
            responses so make sure you select your answers carefully.
            <br />
            Good luck!
          </p>
        </div>
      ...
      <div className={classes.height_control} id="chat_bot_selectors">
        ...
        {/* logout btn */}
        {/* {clog({token,CURRENT_API_URL,company_code,red_url,user_info})} */}
        ...
        <div className={classes.footer_div} data-role="footer">
          <Footer />
        </div>
      </div>
    </div>
